...
We see here the SPI communication of a device doing channel sampling every 125 ms (the narrow bars) and once every second transmit a message (the wide bars). Each bar is actually many SPI transactions for the microcontroller loading the radio with a packet, issuing commands, polling it for status etc.
...
In contrast, unicast transmissions wants an ACK so the radio is kept on and constantly being polled. The trasnmission in the picture was not ACKed so we quit after a while.
