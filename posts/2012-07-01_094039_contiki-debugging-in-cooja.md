...
Either start a new simulation (Ctrl + S) or add more motes to the already existing one. Create a total of *3* nodes with `<a title=\"broadcaster.c source code\" href=\"http://www.bithappens.se/random/contiki-debugging/broadcaster.c\">broadcaster.c</a>` and *7* with `<a title=\"silent.c source code\" href=\"http://www.bithappens.se/random/contiki-debugging/silent.c\">silent.c</a>`.
